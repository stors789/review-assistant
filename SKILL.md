...
4. **Python 依赖安装**：`pip install pymupdf openai requests`
...
默认输出到 `/Users/eros/sciencing/synthesize_output/`，可用 `-o` 覆盖。
...
| `-q` / `--question` | 研究问题（必填） | — |
| `-o` / `--output` | 输出目录 | `/Users/eros/sciencing/synthesize_output` |
...
├── findings/           # Step1 单篇发现 JSON（<hash>.json，同 PDF+同问题可复用）
...
- **双层缓存**：PDF 文本 → SHA256 缓存；API 发现 → (PDF + 问题) 哈希缓存。**换问题才会重新提取**
...
### 文献拆解（能力A）
6. **"拆解 alpha 这个集"** → `python3 scripts/paper_breakdown.py -z "电波 > alpha" -o ./alpha_output -w 5`
7. **"这篇PDF拆解一下"** → `python3 scripts/paper_breakdown.py -i /path/to/folder -o ./out`
8. **"帮我写这篇论文的阅读笔记/摘要"** → `python3 scripts/paper_breakdown.py -i /path/to/pdf/folder -o ./output`，然后读取 JSON
9. **"整理一下 XX 论文集的核心发现"** → 拆解后读取 `_summary.csv`

### 主张验证（能力B）
10. **"帮我验证这段文字"** → `python3 scripts/claim_verify.py "<选集>" -p "文字..."` 或 `-f file.md`
11. **"这段综述里有文献支撑吗"** → `python3 scripts/claim_verify.py "<选集>" -p "..." -o report.json`
12. **"核对一下引用是否准确"** → `python3 scripts/claim_verify.py "<选集>" -f paragraph.md`

### 探索总结（能力D）
13. **"研究一下 alpha 集里关于 XXX 的文献"** → `python3 scripts/explore_synthesize.py "<选集>" -q "XXX"`
...
注意：所有脚本输出默认到 `/Users/eros/sciencing/`，Zotero 不需要关闭。
...
| `No module named 'pymupdf'` | 缺少 Python 依赖 | `pip install pymupdf openai requests` |
...
所有脚本需要 `DEEPSEEK_API_KEY` 或 `OPENAI_API_KEY` 环境变量。用户已配置在 `~/Documents/api.env`，运行前执行：
