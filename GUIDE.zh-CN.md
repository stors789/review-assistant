...
**"研究一下混合检索和重排在 RAG 中的作用"**

给定一个研究问题和一个 Zotero 集，全自动跑完整 7 步流水线：

```bash
# 默认使用常规关键字块匹配与提取：
python scripts/explore_synthesize.py "信息检索 > RAG" -q "混合检索与重排如何提升RAG系统准确性"

# 开启向量语义相似度检索（Hybrid RAG 混合重排排序）：
python scripts/explore_synthesize.py "信息检索 > RAG" -q "混合检索与重排如何提升RAG系统准确性" --vector-search
```
...
**"Zotero 里有多少集？RAG 集有多少 PDF？缺哪些？"**

```bash
python scripts/zotero_read.py --list          # 全集概览
python scripts/zotero_read.py "信息检索 > RAG"  # 单集详情
```
