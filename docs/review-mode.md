...
All AI screening recommendations, record links, extracted fields, quotes, support relations, contradiction groups, drafts, and semantic claim flags require human review. Re-ingesting a publication creates a new extraction run/version; current entities and errors are resolved through `extraction/current_extraction_state.json`, while historical errors remain available and are marked `superseded` when replaced. Scope/population/causality analysis uses structured writer output and configured metadata; ambiguous cases remain `unclear`. A strict pass means the persisted workflow has no detected accounting, linkage, quote, configuration, or structural-audit issue; it is not a proof of scientific correctness or perfect natural-language entailment. The implementation exports PRISMA count data but does not draw a PRISMA diagram. Citation chasing is represented in the search plan but built-in automatic forward/backward chasing is not performed; add named searches or seed records explicitly. Structured JSON is the interchange format for manual extraction corrections.
