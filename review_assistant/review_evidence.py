...
class EvidenceMatrixBuilder:
    ...
    def build(self, row_mode: str = "study") -> list[dict[str, Any]]:
        ...
        if not columns:
            columns = ["publication_id", "study_id", *self.schema.get("fields", {}).keys(), "outcome_domains", "evidence_direction", "quote_validation_status"]
            ...
            for comparison in comparisons or [None]:
                ...
                context = {
                    **study.get("fields", {}), **study,
                    ...
                    "evidence_direction": sorted({item.get("direction", "unclear") for item in outcomes}),
...
class ContradictionAnalyzer:
    ...
    def analyze(self) -> list[dict[str, Any]]:
        ...
        for outcome in outcomes:
            ...
            groups[key].append({"study_id": outcome["study_id"], "outcome_id": outcome["outcome_id"], "direction": outcome.get("direction", "unclear"), "context": context})
        results = []
        for key in sorted(groups):
            ...
            results.append({
                ...
                "has_directional_inconsistency": _inconsistent(directions),
        ...
        for group in results:
            lines.extend([f"## {group['group_id']}", "", f"Studies: {', '.join(group['study_ids']) or 'none'}", f"Directions: {', '.join(group['directions']) or 'none'}", f"Inconsistent: {'yes' if group['has_directional_inconsistency'] else 'no'}", f"Candidate moderators: {_display(group['moderator_candidates'])}", ""])
        atomic_write_text(output / "heterogeneity_report.md", "\n".join(lines))
        return results


def _inconsistent(directions: set[str]) -> bool:
    observed = directions - {"unclear"}
    if "mixed" in observed:
        return True
    positive = bool(observed & {"increase", "positive", "support"})
    negative = bool(observed & {"decrease", "negative", "oppose"})
    no_change = "no_change" in observed
    return (positive and negative) or (no_change and (positive or negative))
